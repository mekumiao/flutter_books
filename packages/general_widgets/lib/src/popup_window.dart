...
Future<T?> showPopupWindow<T>({
  ...
  // 默认位置锚点下方
  var offsetDefault = Offset(0, anchor.size.height);

  if (offset != null) {
    offsetDefault = offsetDefault + offset;
  }

  // 获得控件左下方的坐标
  final a = anchor.localToGlobal(offsetDefault, ancestor: overlay);
  // 获得控件右下方的坐标
  final b = anchor.localToGlobal(
    anchor.size.bottomLeft(offsetDefault),
    ancestor: overlay,
  );
...
class _PopupWindowRoute<T> extends PopupRoute<T> {
  ...
  Animation<double> createAnimation() {
    return CurvedAnimation(
      ...
      curve: Curves.linear,
      reverseCurve: const Interval(0, _kWindowCloseIntervalEnd),
    );
  }
...
class _PopupWindow<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final width = CurveTween(curve: const Interval(0, unit));
    final height = CurveTween(curve: const Interval(0, unit * length));
...
class _PopupWindowLayoutDelegate extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    if (position.left > position.right) {
      ...
      // Menu button is equidistant from both edges,
      // so grow in reading direction.
